...
# Convert data to sf object and transform to match predictors CRS
...
# Extract and save/load land cover class values for 2018
...
# Calculate and filter land cover class counts
...
# Summarize and calculate frequencies
...
# Create frequency table and plot bar chart
...
par(mar = c(3, 20, 3, 2) + 0.1) # Set margins
barplot(freq_table, col = "grey", las = 1, cex.names = 0.58, horiz = TRUE, xlim = c(0, 400))
mtext("frequency", side = 1, line = 2, cex = 0.8)
mtext("land cover class", side = 2, line = 0, cex = 0.8)
abline(v = 100, col = "green", lwd = 2, lty = 2)
abline(v = 200, col = "blue", lwd = 2, lty = 2)
abline(v = 300, col = "purple", lwd = 2, lty = 2)
abline(v = 350, col = "red", lwd = 2, lty = 2)

########################### landcover impervious built-up ######################
# Extract and save/load impervious built-up values
...
# Summarize and calculate statistics
...
# Calculate relative frequencies and create frequency table
relative_freq <- prop.table(freq_table)
print(relative_freq)
freq_table_df <- as.data.frame(freq_table)
...
############################### elevation ######################################
# Extract elevation values and remove NAs
...
# Plot boxplot for elevation values
boxplot(elev_gottan, main = "Boxplot of elevation values at the\nmantis observation points ", ylab = "elevation in m")

############################ surface temperature ###############################
# Extract surface temperature values and convert from Kelvin to Celsius
surftemp_gottan <- mantis_values[ , 4:159] - 273.15
summary(surftemp_gottan)

# Select May data for the years 2010-2022
...
# Calculate minima and means for May data
...
# Create dataframes for minima and means
...
# Create dataframe and boxplot for mean May surface temperature
